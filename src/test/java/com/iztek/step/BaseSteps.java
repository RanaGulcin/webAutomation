...
public class BaseSteps extends BaseTest {
    ...
    Map<String, String> elementTextMap = new HashMap<>();
    ...
    @Step({"Random pick <key>",
            "Rastgele element seç <key>"})
    ...
    @Step("<key> olarak comboboxdan random bir değer seçilir")
    ...
    @Step("<key> elementinin bulunduğu lokasyona git")
    ...
    @Step("<mapKey> mapKeyindeki texti, <key> elementine yaz")
    public void sendKeys(String mapKey, String key) {
        if (!key.equals("")) {
            String getMapKeyText = map.get(mapKey);
            findElement(key).sendKeys(getMapKeyText);
            logger.info(key + " elementine " + getMapKeyText + " texti yazıldı.");
            System.out.println("yazdırıldı:" + getMapKeyText);
        }
    }
